...
function instructions_constructor() {
  ...
  text.append($("<p>").html("In this experiment, we will present you with four kinds of objects from an alien planet. You will be shown examples and asked to guess whether each example is a kind of object. Then you will be asked how each kind relates to the other. It should take about 10 minutes."))
  text.append($("<p>").html("(Note: you won't be able to preview this HIT before accepting it.)"))
  text.append($("<p>").html("By answering the following questions, you are participating in a study being performed by cognitive scientists in the University of Louisville Department of Brain and Psychological Sciences. If you have questions about this research, please contact us at s@sophiaray.info . You must be at least 18 years old to participate. Your participation in this research is voluntary. You may decline to answer any or all of the following questions. You may decline further participation, at any time, without adverse consequences. Your anonymity is assured; the researchers who have requested your participation will not receive any personal information about you."))
...
function instructions_constructor_2() {
  ...
  text.append($("<p>").html("Now you will see different kinds of objects from an alien planet. You will be asked to identify each example and given feedback to help you learn."))
...
function relational_slide(test_id) {
  ...
  for (pair of _.shuffle(shape_pairs)) {
    ...
    shape1_plural = exp.pseudo_obj[shape1][1]
    shape2_plural = exp.pseudo_obj[shape2][1]
    if (shape1 != shape2) {
      row = $("<tr>")
...
function training_constructor() {
  ...
  // display question
  slide.append($("<p class='training_text'>").html(`Is this a ${question_text}?`));
  // display y/n buttons
  ...
  slide.append($(`<div class='btn-group' >`),
...
function generate_shape(shape_name) {
  feature_values = {
    "square": [1,1],
    "rectangle": [1,0],
    "rhombus": [0,1],
  ...
  border = exp.border_values[feature_values[0]]
  pattern = exp.pattern_values[feature_values[1]]
  return abstract_text(border,pattern)
}
...
function abstract_text(border,pattern) {
  ...
  scale = 30
  points_text = ""
  for (point_pair of points) {
    points_text += String(scale*point_pair[0]) + "," + String(scale*point_pair[1]) + " "
  }
  poly.setAttribute('class','circles-1')
  poly.setAttribute('points',points_text)
  if (pattern=="circle"){
    poly.setAttribute('style','fill: url(#circles-7) #fff;')
  }
  else if (pattern=="square"){
    poly.setAttribute('style','fill: url(#dots-7) #fff;')
  }

  poly.setAttribute('stroke','DarkSlateGray')
  if (border=="dash"){
