...
ms <- d1 %>% 
  ...
  group_by(question_and_shape, question, block, training_condition, experiment) %>%
...
ggplot(aes(x = question_and_shape, y = m, fill = block), data = ms) + 
    ...
    ylim(0, 1) + 
    facet_grid(training_condition~experiment) +
...
ms <- d1 %>% 
  ...
  group_by(question_and_shape, question, training_condition, block, experiment) %>%
  summarise(m.cih = ci.high(correct),
            m.cil = ci.low(correct),
            m = mean(correct))
...
ms <- d1 %>% 
    filter(trial_type %in% c("relational", "entity")) %>% 
    group_by(training_condition, trial_type, block = as.factor(block), experiment) %>%
...
ss_acc <- d1 %>% 
    filter(trial_type %in% c("relational", "entity")) %>% 
    group_by(subids, training_condition, block, trial_type, experiment) %>% 
    summarise(m_acc = mean(correct)) %>% 
    spread(key = block, value = m_acc) %>% 
    mutate(m_diff_score = posttest - pretest)

ms_change <- ss_acc %>% 
    group_by(training_condition, trial_type, experiment) %>% 
...
### Within subjects change scores for learned shape
