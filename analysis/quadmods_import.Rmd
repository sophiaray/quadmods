...
```{r}
setwd("../raw-data/pilot-a/production-results")
```
           ...
           question = ifelse(question == "rectangles", "rectangle", 
                          ifelse(question == "rhombuses", "rhombus", 
                                 ifelse(question == "squares", "square",
                                        ifelse(question == "parallelograms", "parallelogram", question)))),
...
flag_correct <- function (trial) {
    trial_type <- as.character(trial[which(names(trial)=="trial_type")])
    ...
    # retun T/F for each trial
    correct <- response == correct_answer
     return(correct)
}

# create a vector of correct/incorrect
correct <- apply(df_final, 1, flag_correct)
...
```{r}
write.csv(x = df_final_clean, file = "../../../data/quadmods-sequences-1.csv", row.names=F)
```
